...
/*    g.selectAll("rect")
  ...
  drawChart(state) {
    ...
    let { br, saude_familia } = state;

    if ("objects" in br && this.drawed === false) {

      this.drawed = true;

      this.data = Object.assign(new Map(saude_familia), { title: "Saúde da Família - Número de equipes" });
      ...
      const gequipes = this.svg.append("g")
        .attr("id", "gequipes");

      gequipes.selectAll("path")
        .data(topojson.feature(br, br.objects.DADOS).features)
        .join("path")
        .attr("fill", "brown") //("fill", d => this.color(this.data.get(`${d.id.slice(0, 2)}-${d.properties.name}`)))
